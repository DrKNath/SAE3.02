...
Sur la partie de gauche vous trouverais les différent zone a renseigner comme le nom du client, le port d'écoute, l'ip du master et son port.

Un fois que tout est correctement renseigné vous pouvez lancé le client avec le bouton ```Lancer le Client```.
Si vous renseigné mal les information ou que vous vous trompé dans ip/ port master ou alors que le port d'écoute que vous voulez utilisé est déja pris il sera indiqué dans la console la où vous avec fait ```python -m client.main```, si un message apparait dans cette derniere vérifier les information que vous avez renseigné, et rappuyer sur le bouton ```Lancer le Client```, ce pendant si vous changé des information pensé à changé le port d'écoute.
